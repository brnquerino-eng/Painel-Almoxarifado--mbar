...
with aba_geral:
    ...
    with st.container(border=True):
        ...
        # OTIMIZAÇÃO/CORREÇÃO: usa o parâmetro nativo type="primary"/"secondary"
        # do st.button para o destaque visual, em vez de injetar CSS via
        # button[key='...'] — key não vira atributo HTML no navegador, então
        # aquele seletor nunca casava com nada e o destaque não aparecia.
    ...
    # OTIMIZAÇÃO: esse cálculo (consumo_abs, val_estoque, máscara
    # crítico/obsoleto, soma por mês) era refeito do zero em três lugares:
    # aqui para o período atual, de novo para o mês anterior (setinhas), e
    # de novo mais abaixo para o gráfico "Giro vs Cobertura". Como o
    # agrupamento por mês não muda entre essas três necessidades — só a
    # janela de meses que cada uma olha depois é diferente — agora ele roda
    # uma única vez sobre toda a base filtrada, gerando uma tabelinha (uma
    # linha por mês) que os três usos abaixo reaproveitam.
    ...
    if not df_filtrado.empty:
        ...
        with col_c1:
            with st.container(border=True):
                ...
                # CORREÇÃO/OTIMIZAÇÃO: usa st.plotly_chart com a altura dinâmica
                # aplicada de fato (height=altura_grafico), em vez de exportar
                # via to_html()+components.v1.html(height=380 fixo) — aquele
                # hack recarregava o Plotly.js inteiro via CDN a cada rerun
                # (inclusive ao clicar num botão de legenda) e a altura fixa
                # cortava listas com mais de ~12 unidades, exigindo rolagem.
        ...
        with st.container(border=True):
            ...
            # OTIMIZAÇÃO: reaproveita a tabela monthly_raw já calculada acima
            # em vez de recomputar consumo_abs/val_estoque/máscara/estoque_op/
            # consumo_op do zero sobre toda a base filtrada de novo — era a
            # terceira vez que esse mesmo trabalho pesado era feito no script.
